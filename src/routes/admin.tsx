...
import { useAdminAuth, useCms, SaveStatus } from "@/store/cms";
import { supabase } from "@/integrations/supabase/client";
import { ImageDropzone } from "@/components/admin/ImageDropzone";
import { Toaster, toast } from "sonner";
import {
  LogOut, Plus, Trash2, Save, Lock, Home, RotateCcw,
  Eye, EyeOff, CheckCircle2, AlertCircle, Loader2, AlertTriangle,
} from "lucide-react";
...
// Credentials are read from env vars at build time.
// In .env: VITE_ADMIN_LOGIN, VITE_ADMIN_PASSWORD, VITE_ADMIN_EMAIL
// Fallback values kept so the app still works if env vars are missing.
const ADMIN_LOGIN = import.meta.env.VITE_ADMIN_LOGIN ?? "admin";
const ADMIN_PASSWORD = import.meta.env.VITE_ADMIN_PASSWORD ?? "emin.admin07";
const ADMIN_EMAIL = import.meta.env.VITE_ADMIN_EMAIL ?? "admin@msk-asfalt.local";
...
function SaveBadge({ status }: { status: SaveStatus }) {
  ...
  const map: Record<Exclude<SaveStatus, "idle">, { icon: React.ReactNode; label: string; cls: string }> = {
    saving: { icon: <Loader2 className="size-4 animate-spin" />, label: "Сохраняю…", cls: "text-muted-foreground" },
    saved: { icon: <CheckCircle2 className="size-4 text-primary" />, label: "Сохранено", cls: "text-primary" },
    error: { icon: <AlertCircle className="size-4 text-destructive" />, label: "Ошибка сохранения", cls: "text-destructive" },
  };
...
function AdminDashboard() {
  ...
  const dirty = useCms((s) => s.dirty);
  const saveStatus = useCms((s) => s.saveStatus);
  const saveNow = useCms((s) => s.saveNow);
  ...
  // Warn before tab/browser close if dirty
  useEffect(() => {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (useCms.getState().saveStatus === "saved") {
      toast.success("Изменения сохранены и опубликованы на сайте!");
    } else {
  ...
  return (
    <div>
      <header className="bg-card border-b border-border sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {dirty && saveStatus === "idle" && (
              <span className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-600 dark:text-amber-400">
                <AlertTriangle className="size-4" /> Есть изменения
              </span>
            )}
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold transition-all shadow-soft ${
                dirty && saveStatus !== "saving"
                  ? "bg-gradient-brand text-white hover:shadow-glow-green hover:-translate-y-0.5"
            ...
            <button
              onClick={() => {
                if (confirm("Сбросить все данные к исходным? Нажмите «Сохранить» после сброса чтобы применить.")) {
                  reset();
                  toast.info("Данные сброшены. Нажмите «Сохранить» для публикации.");
                }
              }}
        ...
        {dirty && (
          <div className="bg-amber-50 dark:bg-amber-950/40 border-t border-amber-200 dark:border-amber-800 px-4 py-2 flex items-center justify-between gap-3">
            <span className="text-xs text-amber-700 dark:text-amber-300 font-medium">
              ⚠ У вас есть несохранённые изменения. Нажмите «Сохранить», чтобы опубликовать их на сайте.
            </span>
            <button
              onClick={handleSave}
              disabled={saveStatus === "saving"}
              className="shrink-0 rounded-full bg-amber-600 text-white px-4 py-1.5 text-xs font-semibold hover:bg-amber-700 transition disabled:opacity-60"
            >
              {saveStatus === "saving" ? "Сохраняю…" : "Сохранить сейчас"}
            </button>
          </div>
        )}
      ...
      <main className="container-x py-8 pb-32">
      ...
      {dirty && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
          <button
            onClick={handleSave}
            disabled={saveStatus === "saving"}
            className="pointer-events-auto inline-flex items-center gap-2 rounded-full bg-gradient-brand text-white px-6 py-3 text-sm font-bold shadow-[0_8px_32px_rgba(33,160,56,0.45)] hover:shadow-glow-green hover:-translate-y-0.5 transition-all disabled:opacity-60"
          >
...
function ContactsEditor() {
  ...
  return (
    <Card title="Контакты">
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="Телефон (для ссылки, напр. +74951234567)" value={c.phone} onChange={(v) => set({ phone: v })} />
        <Field label="Телефон (отображение)" value={c.phoneDisplay} onChange={(v) => set({ phoneDisplay: v })} />
        <Field label="WhatsApp (номер, напр. +79001234567)" value={c.whatsapp ?? ""} onChange={(v) => set({ whatsapp: v })} />
        <Field label="Telegram (username без @)" value={c.telegram ?? ""} onChange={(v) => set({ telegram: v })} />
